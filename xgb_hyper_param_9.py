...
#starting parameters
param_test ={    
            'eta': sp_uniform(loc=0.0, scale=1.0),
            'max_depth': sp_randint(1, 10),
            'min_child_weight': sp_uniform(loc=0, scale=1.0),
# =============================================================================
#             'subsample': sp_uniform(loc=0.2, scale=0.8), 
#             'gamma': sp_uniform(loc=0.2, scale=0.8), 
...
xgb_reg = xgb.XGBRegressor(random_state=14, silent=False, booster='gbtree', eval_metric='mae', objective='reg:gamma', n_jobs=1, n_estimators=50000, bagging_seed=1)
gs = RandomizedSearchCV(estimator=xgb_reg, param_distributions=param_test, n_iter=n_hyper_parameter_points_to_test, fit_params=fit_params,
...
#master model
def train_model(X=X_train_scaled, X_test=X_sub_scaled, y=y_tr, params=None, folds=folds, model_type='xgb', plot_feature_importance=False, model=None):
...
#xgb
xgb_params = {'max_depth': 4, 'min_data_in_leaf': 2, 'num_leaves': 321,
...
oof_xgb, prediction_xgb, feature_importance = train_model(params=xgb_params, model_type='xgb', plot_feature_importance=True)
...
# submission['time_to_failure'] = prediction_xgb_stack
